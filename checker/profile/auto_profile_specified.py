...
def main():
    ...
    if os.path.basename(__file__) == "profile.py":
        print("❌ [Error] Please rename this script to 'benchmark.py'. Do NOT use 'profile.py'!")
        return

    if not os.path.exists("./run.sh"):
        print("❌ [Error] Please run this script inside the 'checker' directory.")
        return

    results = []
    total_tasks = len(SIZES) * len(P_VALUES) * len(DTYPES)
    
    print(f"🚀 Starting automation test, total {total_tasks} cases...")
